<div align='center'>
  <h1 align='center'>Guard Notify API</h1>
  <h3 align='center'>Email Notification API for Guard</h3>
</div>
...
```javascript
guard.notifyAuthorize('stargaze')
```

In this case, the app ID is `stargaze`. When provisioning your app with us, you will receive an API key and an app ID. **NEVER** store the API key on the frontend or share it with a third party. Only use it when interacting with the Guard API on the backend.
